...
async function updateSystemMetrics() {
  ...
  try {
    ...
    console.log(
      `📊 [${new Date().toISOString()}] Updating metrics for ${agents.length} agents`,
    );
    ...
    for (const agent of agents) {
      ...
      const hasRealMetrics =
        ...
        new Date(agent.systemMetrics.lastUpdated) >
          new Date(Date.now() - 2 * 60 * 1000);

      if (hasRealMetrics) {
        console.log(
          `  ✅ ${agent.name}: Sending real metrics from updated agent`,
        );
        continue;
      }
      ...
      console.log(
        `  🔄 ${agent.name}: Updated simulated metrics (CPU=${systemMetrics.cpuUsagePercent.toFixed(1)}%, Load=${loadScore})`,
      );
      updatedCount++;
    }
...
function generateSystemMetrics(agent) {
  ...
  const finalLoad = Math.max(
    0.05,
    baseLoad * performanceMultiplier * ageMultiplier,
  );

  return {
    cpuUsagePercent: Math.min(finalLoad * 120 + Math.random() * 8, 95),
    memoryUsagePercent: Math.min(finalLoad * 100 + Math.random() * 10 + 25, 90),
    memoryUsedBytes: Math.floor(
      (finalLoad * 0.7 + 0.3) * 8 * 1024 * 1024 * 1024,
    ),
...
console.log(
  "📝 This will update metrics every 60 seconds until agents are updated",
);
