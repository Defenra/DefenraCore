...
async function forceSystemMetrics() {
  try {
    ...
    for (const agent of agents) {
      ...
      console.log(
        `  ✅ Updated metrics: CPU=${systemMetrics.cpuUsagePercent.toFixed(1)}%, Memory=${systemMetrics.memoryUsagePercent.toFixed(1)}%, Load=${loadScore}`,
      );
    }
...
function generateSystemMetrics(agent) {
  ...
  return {
    ...
    memoryUsedBytes: Math.floor(
      (finalLoad * 0.8 + 0.2) * 8 * 1024 * 1024 * 1024,
    ), // 1.6-6.4GB used
