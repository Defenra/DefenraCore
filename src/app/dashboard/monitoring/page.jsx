...
function Panel({
  ...
  return (
    <Card
      className={cn(
        "border border-border/40 bg-card/60 overflow-hidden",
        className,
      )}
    >
...
export default function MonitoringPage() {
    ...
    if (metricsData?.timeSeries?.length && agentsToShow.length) {
      return metricsData.timeSeries.map((point) => {
        ...
        agentsToShow.forEach((agent) => {
          const agentMetrics = point.agents?.find(
            (a) => a.agentId === agent.agentId,
          );
          dataPoint[agent.name || agent.agentId] =
            agentMetrics?.loadScore || agent.loadScore || 0;
        });
  ...
  const trafficData = useMemo(() => {
    ...
    if (metricsData?.timeSeries?.length) {
      return metricsData.timeSeries.map((point) => ({
        time: new Date(point.time || point.timestamp).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
    ...
    if (metricsData?.topAgents?.length) {
      ...
      const totalRequests = sortedTopAgents.reduce(
        (sum, a) => sum + (a.requests || 0),
        0,
      );
      ...
      return sortedTopAgents.map((agent) => {
        ...
        const requestShare =
          totalRequests > 0 ? (agent.requests || 0) / totalRequests : 0;
  ...
  const stats = useMemo(() => {
    ...
    const avgLoad =
      agents.length > 0
        ? Math.floor(
            agents.reduce((s, a) => s + (a.loadScore || 0), 0) / agents.length,
          )
        : 0;

    // Get latest metrics point
    const latestPoint =
      metricsData?.timeSeries?.[metricsData.timeSeries.length - 1];
    ...
    if (metricsData?.timeSeries && metricsData.timeSeries.length > 0) {
      ...
      const totalRequests = metricsData.timeSeries.reduce(
        (sum, point) => sum + (point.requests || 0),
        0,
      );
      const timeRangeMinutes =
        timeRange === "1h" ? 60 : timeRange === "24h" ? 1440 : 10080; // 7d
      ...
      const timeRangeMinutes =
        timeRange === "1h" ? 60 : timeRange === "24h" ? 1444 : 10080;
      requestsPerMinute = Math.round(
        metricsData.stats.requests / timeRangeMinutes,
      );
    }
    ...
    if (bansData?.bans) {
      ...
      const bansLastMinute = bansData.bans.filter((ban) => {
        const banTime = new Date(
          ban.bannedAt || ban.createdAt || ban.timestamp,
        );
    ...
    return {
      ...
      blockedPerMinute,
      activeUsers:
        (metricsData?.stats?.requests || 0) -
        (metricsData?.stats?.blockedRequests || 0),
    ...
    <div className="flex flex-col gap-6">
      ...
      {metricsError && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {t("monitoring.errors.loadFailed")}: {metricsError.message}. Showing
            agent data only.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Panel
          ...
          emptyMessage={
            hasAgents
              ? t("monitoring.empty.collecting")
              : t("monitoring.empty.noAgents")
          }
          actions={
        ...
        <Panel
          title={t("monitoring.panels.topAgents")}
          loading={agentsLoading}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(agent.loadScore || 0, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <Panel
          title={t("monitoring.panels.agentHealth")}
          loading={agentsLoading}
          empty={agents.length === 0 && !agentsLoading}
        >
